...
1. [Offline Storage and Automatic Retries](https://learn.microsoft.com/en-us/azure/azure-monitor/app/opentelemetry-configuration?tabs=net#offline-storage-and-automatic-retries) are enabled by default in the [Azure Monitor OpenTelemetry Distro](https://learn.microsoft.com/en-us/azure/azure-monitor/app/opentelemetry-enable?tabs=aspnetcore), but [isn't a feature of the base .NET OpenTelemetry implementation yet](https://github.com/open-telemetry/opentelemetry-dotnet/issues/4115). Nonetheless this will guarantee **eventual export of logs** to Azure Monitor. 
1. [ForceFlush](https://opentelemetry.io/docs/specs/otel/trace/sdk/#forceflush) is implemented via `AutoFlushActivityProcessor` and guarantees **on time export of spans** to Azure Monitor. This processor is in [OpenTelemetry.Extensions](https://www.nuget.org/packages/OpenTelemetry.Extensions). See https://github.com/open-telemetry/opentelemetry-specification/issues/2944#issuecomment-1319536765 for more context. It doesn't check the flush status but does wait until it's done, please see: https://github.com/open-telemetry/opentelemetry-dotnet-contrib/issues/2721.
