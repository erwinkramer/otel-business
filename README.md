...
1. **No guarantee in a disaster.** In case of a disaster (unexpected shutdown, hardware failure, stack overflow etc.) the reliability of OTel (at the moment) is very low, even with the reliability measurements taken as described earlier. As a demonstration, please see the `FailFastTest` in [IntegrationTest](/IntegrationTest/), this test will not produce any trace or span data in Azure Monitor. 
